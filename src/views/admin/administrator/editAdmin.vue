  ...
  <div id="administratorEditAdmin">
    <el-form :model="administratorInfo" label-width="240px" :rules="administratorInfoRules" ref="administratorInfoForm">
      ...
      <el-form-item label='姓名：' prop="username">
        <el-input v-model="administratorInfo.username" placeholder="请输入内容" style="width: 150px;"></el-input>
      </el-form-item>
...
//        ],
    ...
    handleSubmit() {
      this.$refs.administratorInfoForm.validate((valid) => {
        if (valid) {
          ...
          this.$http.post('/admin/modify', {
            id: this.$route.params.id,
            rolesId: this.administratorInfo.rolesId,
            avatar: this.administratorInfo.avatar,
            name: this.administratorInfo.username,
  ...
  mounted() {
    ...
    this.$http.get(`/admin/user/detail?id=${this.$route.params.id}`).then((response) => {
      if (response.data.errorCode === 10000) {
        this.administratorInfo = response.data.data;
      }
    });
  },
};
