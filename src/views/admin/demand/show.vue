		...
		<el-tabs v-model="defaultTab" type="card" @tab-click="handleTabClick">
			...
			<el-tab-pane v-if="demandInfo.status!=0 && demandInfo.status != 3 && demandInfo.status!=4 && demandInfo.status!=2" label="发布计划" name="second">
				<el-form :model="planInfo" lable-width="200px" :rules="planRules" ref="planForm" class="publish">
					...
					<el-form-item label='满返：' v-for="(scheme, index) in planInfo.schemes" style="margin-top: 20px;">
						...
						<el-col :span="4">
							<el-input-number v-model="scheme.limitDay" :min="0" :max="100000" style="width: 150px;"></el-input-number>
						</el-col>
						<el-col :span="3" style="font-size: 20px; margin-left: 25px;">返（元）</el-col>
						<el-col :span="4">
							<el-input-number v-model="scheme.amount" :min="0" :max="100000" style="width: 150px;"></el-input-number>
						</el-col>
					</el-form-item>
					...
					<el-form-item label='提成方式：'>
						<el-col :span="5">
							<el-select v-model="planInfo.commissionType" placeholder="请选择">
								<el-option v-for="type in chargeType" :label="type.label" :value="type.value">
								</el-option>
							</el-select>
						</el-col>
					...
					<el-form-item label='截止日期：'>
						<el-date-picker v-model="planInfo.endTime" type="datetime" placeholder="选择日期时间">
						</el-date-picker>
					</el-form-item>
					<el-form-item>
			...
			<el-tab-pane v-if="demandInfo.status!=0 && demandInfo.status!=4 && demandInfo.status!=2" label="招聘计划" name="fourth">
				<el-table :data="plans" border style="width: 100%" v-loading="loading">
					...
					<el-table-column label="计划名称" width="150" align="center" prop="name">
					</el-table-column>
					<el-table-column label="类型" width="100" align="center" :formatter="formatPlanType" prop="type">
					</el-table-column>
					<el-table-column label="发布时间" width="180" align="center" :formatter="formatStartTime" prop="created_at">
					</el-table-column>
					<el-table-column label="截止时间" width="180" align="center" :formatter="formatEndTime" prop="end_time">
					</el-table-column>
					<el-table-column label="招聘人数" width="100" align="center" prop="alreadyNumber">
					</el-table-column>
					<el-table-column label="状态" width="100" :formatter="formatPlanStatus" align="center" prop="status">
					</el-table-column>
					<el-table-column label="操作" align="center">
						<template scope="scope">
							<el-button v-if="scope.row.status === 1" size="small" @click="handleRefush(scope.$index, scope.row)">刷新</el-button>
							<el-button v-if="scope.row.status === 1" type="danger" size="small" @click="handleEarlyEnd(scope.$index, scope.row)">提前结束</el-button>
						</template>
					</el-table-column>
				</el-table>
			...
			<div class="search-table">
				<el-table :data="planInfo.brokerSearchData" v-loading="planInfo.loading">
					...
					<el-table-column label="操作">
						<template scope="scope">
							<el-button size="small" @click="handleConfirmBroker(scope.$index, scope.row)">选择</el-button>
						</template>
					</el-table-column>
				</el-table>
		...
		methods: {
			...
			publishPlan() {
				...
				if(this.planInfo.submitPrice !== 0) {
					allCommission.push({
						commission_id: 1,
						amount: this.planInfo.submitPrice
					});
				}
				if(this.planInfo.interviewAlreadyPrice !== 0) {
					allCommission.push({
						commission_id: 2,
						amount: this.planInfo.interviewAlreadyPrice
					});
				}
				if(this.planInfo.interviewSuccessPrice !== 0) {
					allCommission.push({
						commission_id: 3,
						amount: this.planInfo.interviewSuccessPrice
					});
				}
				if(this.planInfo.workSuccessPrice !== 0) {
					allCommission.push({
						commission_id: 4,
						amount: this.planInfo.workSuccessPrice
					});
				}
				...
				if(this.planInfo.commissionType !== '0') {
					...
					if(this.planInfo.chargePrice !== 0) {
						if(selectId === 6) {
							allCommission.push({
								commission_id: selectId,
								percentage: this.planInfo.chargePrice
							});
						} else {
							allCommission.push({
								commission_id: selectId,
								amount: this.planInfo.chargePrice
							});
						}
					}
				}
