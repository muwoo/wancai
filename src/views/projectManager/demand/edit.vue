<template>
  <div id="projectManagerDemandEdit" v-loading="loading">
    <!-- 需求表表单 -->
  ...
  export default {
    ...
    methods: {
      ...
      handlePublish() {
        ...
        this.$http.post(`${this.$managerURL}/demand/modify`, params).then((response) => {
          if (response.data.errorCode === 10000) {
            this.$notify({
              title: '修改成功',
              type: 'success',
            });
            this.$router.push({ name: 'projectManagerDemandShow', params: { id: this.$route.params.id }, query: { id: this.$route.query.id } });
          } else {
            this.$notify.error({
              title: '修改异常',
              type: 'success',
